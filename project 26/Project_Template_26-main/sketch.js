...
function keyPressed() {
  ...
  if(keyCode === 32){
    ...
    var array = new PlayerArrow(posX, posY, 100, 10);
    arrow.trajectory = [];
...
function keyReleased () {
  ...
  if(keyCode === 32){
    ...
    if (playerArrows.length) {
      ...
      var angle = playerArcher.body.angle+PI/2;
      playerArray[playerArray.length - 1].shoot(angle);
...
function showArrows(index, arrows) {
  ...
  function showArray(index, array) {
    array [index].display();
...
{ Matter.SAT.collides(bodyA,bodyB,previousarrowCollision) 
}
